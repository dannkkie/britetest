...
class TestMovieResource(unittest.TestCase):
    ...
    def test_add_movie_to_db(self):
        ...
        self.assertEqual(response.json["movie"]["title"], "The Shawshank Redemption")
    ...
    def test_delete_movie_by_id_with_authorization(self):
        ...
        response = self.client.delete(f"/api/v1/movies/{movie_id}", headers=headers)
        self.assertEqual(response.status_code, 200)
    ...
    def test_delete_nonexistent_movie_by_id_with_authorization(self):
        ...
        headers = {"Authorization": f"Bearer {access_token}"}
        response = self.client.delete("/api/v1/movies/tt0000000", headers=headers)
